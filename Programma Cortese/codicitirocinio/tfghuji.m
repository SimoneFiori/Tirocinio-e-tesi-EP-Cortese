...
%     %asse X
%     ax1 = subplot(311); 
%     plot(psi(1,:)*(180/pi),'r');  title('Dati da smartphone'); grid on; xlabel('Istanti'); ylabel('pitch');
%     ylim(ax1,[-100,100]);
% 
%     %asse Y
%     ax2 = subplot(312);
%     plot(180*psi(2,:)/pi,'g'); grid on; xlabel('Istanti'); grid on; ylabel('roll');
%     ylim(ax2,[-100,100]);
% 
%        %asse Z
%        ax3 = subplot(313);
%        plot(psi(3,:)*(180/pi),'k'); grid on; xlabel('Istanti'); grid on; ylabel('yaw');
%        ylim(ax3,[-10,370]);
% 
%     pause(0.001);
